...
func (h *Handler) Register() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var req api.PostRegisterJSONRequestBody
		...
		_, err = h.authService.Register(r.Context(), string(req.Email), req.Password, models.UserRole(req.Role))
		if err != nil {
			log.Error("failed to register", sl.Err(err))

			w.WriteHeader(http.StatusInternalServerError)
			render.JSON(w, r, api.Error{Message: "failed to register"})

			return
		}

		w.WriteHeader(http.StatusCreated)
		render.JSON(w, r, api.User{Email: req.Email, Role: api.UserRole(req.Role)})
	}
}
